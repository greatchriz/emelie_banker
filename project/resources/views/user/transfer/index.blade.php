                                ...
                                <tbody>
                                  @foreach($logs as $key=>$data)
                                      <tr>
                                          ...
                                          @if ($data->receiver_id)
                                            <td data-label="{{ __('Account No') }}">{{ $data->receiver->account_number ?? 'User Deleted' }}</td>
                                            <td data-label="{{ __('Account Name') }}">{{ $data->receiver->name ?? 'User Deleted' }}</td>
                                          @endif

                                          @if (!$data->receiver_id)
                                            <td data-label="{{ __('Account No') }}">{{ $data->beneficiary->account_number ?? 'deleted' }}</td>
                                            <td data-label="{{ __('Account Name') }}">{{ $data->beneficiary->account_name ?? 'deleted' }}</td>
                                          @endif
                                          <td data-label="{{ __('Type') }}">{{ $data->type }} {{ __('Bank') }}</td>
                                          <td data-label="{{ __('Amount') }}">{{ showprice($data->amount,$currency) }}</td>
                                          ...
                                          <td data-label="{{ __('Action') }}">
                                            <a href="{{ route('transfer.logs.show', $data->id) }}" class="btn btn-primary btn-sm">
                                              {{ __('Details') }}
                                            </a>
                                          </td>
                                      </tr>
